...
// File: contracts/interfaces/IQuickswapRouter01.sol
...
// File: contracts/interfaces/IQuickswapRouter02.sol
...
// File: contracts/interfaces/IQuickswapPair02.sol
...
// File: contracts/libraries/QuickswapLibrary.sol
...
// File: contracts/QuickswapRouter02.sol
contract QuickswapRouter is IQuickswapRouter02 {
    using SafeMath for uint;
    ...
    function removeLiquidity(
        ...
        if(amountA < amountAMin) revert Insufficient_A_Amount();
        if(amountB < amountBMin) revert Insufficient_B_Amount();
    }
    ...
    function swapExactTokensForTokens(
        ...
        if (amounts[amounts.length - 1] < amountOutMin) revert Insufficient_Output_Amount();
    ...
    function swapTokensForExactTokens(
        ...
        if (amounts[0] > amountInMax) revert Excessive_Input_Amount();
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, QuickswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
